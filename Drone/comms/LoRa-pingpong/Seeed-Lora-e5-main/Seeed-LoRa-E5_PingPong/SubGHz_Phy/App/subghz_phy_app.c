...
void SubghzApp_Init(void)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_M, "Radio parameters set, waiting for the radio\n");
  Radio.Init(&RadioEvents);
  APP_LOG(TS_ON, VLEVEL_M, "Radio initialized\n");
  /* USER CODE BEGIN SubghzApp_Init_2 */
  /*calculate random delay for synchronization*/
  random_delay = (Radio.Random()) >> 22; /*10bits random e.g. from 0 to 1023 ms*/
  APP_LOG(TS_ON, VLEVEL_M, "Generated random delay %d\n", random_delay);
  /* Radio Set frequency */
  Radio.SetChannel(RF_FREQUENCY);
  APP_LOG(TS_ON, VLEVEL_M, "Set frequency to %d\n", RF_FREQUENCY);
...
#error "Please define a modulation in the subghz_phy_app.h file."
  ...
  APP_LOG(TS_ON, VLEVEL_M, "Tx buffer filled\n");
  APP_LOG(TS_ON, VLEVEL_L, "rand=%d\n\r", random_delay);
  /*starts reception*/
  Radio.Rx(RX_TIMEOUT_VALUE + random_delay);
  APP_LOG(TS_ON, VLEVEL_M, "Running mock ping pong\n");
  PingPong_Process();
